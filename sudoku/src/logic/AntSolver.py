...
class AntSolver:
    def __init__(
        ...
        self.pher_matrix = [
            [
                [self.initial_pher_val for _ in range(self.grid.grid_size)]
    ...
    def printExecTime(self):
        print(f"Execution Time : {round(self.exectime, 8)} seconds")
        ...
        for row in range(self.grid.grid_size):
            for col in range(self.grid.grid_size):
                ...
                if not sol_cell.failed():
                    self.pher_matrix[row][col][sol_cell.get_val() - 1] = (
                        self.pher_matrix[row][col][sol_cell.get_val() - 1]
                        * (1 - self.global_pher_update)
                        + self.global_pher_update * self.best_pher_to_add
                    )
    ...
    def solve(self, local_pher_update, greediness):
        ...
        while not solved:
            ...
            for i in range(self.num_of_ants):
                ...
                start_pos = (
                    random.randint(0, self.grid.grid_size - 1),
                    random.randint(0, self.grid.grid_size - 1),
                )
            ...
            for idx, ant in enumerate(self.ants):
                ...
                if num_fixed == self.grid.cell_cnt:
                    ...
                    if self.gui_active:
                        self.gui.screen_with_message(
                            self.solution, "", False, cycle, self.pher_matrix
                        )
                    else:
                        self.solution.print()
                        print(self.solution.fixed_cell_cnt, "fixed cells")
                        # self.print_pher_matrix()
                    self.exectime = time.time() - st
